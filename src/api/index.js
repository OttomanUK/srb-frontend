...
export const getAllNtn = async () => await API.get(`/filter?ntn=all`);
export const getAllInvoice = async (page,anomaly,date) => await API.get(`/filter?anomaly=${10}&page=${page}&date=${date}`);
export const getNtnInvoice = async (id,page,anomaly,date) => await API.get(`filter?anomaly=${10}&anomaly_by_ntn=${id}&page=${page}&date=${date}`);
export const getPosInvoice = async (id,ntn,page,anomaly,date) => await API.get(`/filter?anomaly=${10}&anomaly_by_pos=${id}&anomaly_by_ntn=${ntn}&page=${page}&anomaly_by_date=2023-11-18`);
export const getNtnPos = async (id) => await API.get(`/NtnPos/${id}`);
export const getMissingInvoice = async (id,page) => await API.get(`/filter?missing_invoice_by_ntn=all&page=${page}`);
