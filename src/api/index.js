...
export const getSingleInvoice = async (id) => await API.get(`/filter?anomaly=True&anomaly_by_srb_invoice_id=${id}`);
...
export const getPosInvoice = async (id,ntn,page,anomaly,date) => await API.get(`/filter?anomaly=${anomaly}&anomaly_by_pos=${id}&anomaly_by_ntn=${ntn}&page=${page}&date=${date}`);
